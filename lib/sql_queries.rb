...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "select title, sum(amount) - funding_goal from projects inner join pledges on projects.id = pledges.project_id group by title where sum(amount) - funding_goal >= 0"
end
